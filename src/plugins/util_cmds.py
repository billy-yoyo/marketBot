    ...
    try:
        if cmd[0] == "tag":
            if is_tag_banned(bot, msg) and not bot.is_me(msg):
                ...
                if cmd[1] == "ban":
                    if bot.is_me(msg) or msg.channel.permissions_for(msg.author).kick_members:
                        if len(msg.mentions) > 0:
                            for member in msg.mentions:
                                if not msg.channel.id in bot.market.tags["__tagban__"]:
                                    bot.market.tags["__tagban__"][msg.channel.id] = []
                                if not msg.author.id in bot.market.tags["__tagban__"][msg.channel.id]:
                                    bot.market.tags["__tagban__"][msg.channel.id].append(msg.author.id)
